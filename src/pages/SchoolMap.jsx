  ...
  const fetchData = async () => {
    try {
      const [visitsRes, schoolsRes, personnelRes] = await Promise.all([
        fetch("http://localhost:3002/api/visits").then(res => res.json()),
        fetch("http://localhost:3002/api/schools").then(res => res.json()),
        fetch("http://localhost:3002/api/marketing-personnel/").then(res => res.json()),
      ]);
...
const handleAssignVisit = async () => {
  try {
    await fetch("http://localhost:3002/api/visits", {
      method: "POST",
  ...
  const handleEditVisit = async () => {
    try {
      await fetch(`http://localhost:3002/api/visits/${editVisit.visit_id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
