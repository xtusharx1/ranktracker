...
const Course = () => {
  ...
  const handleAddStudentsToBatch = async () => {
    try {
      ...
      for (const user_id of selectedStudents) {
        await axios.post(`http://localhost:3002/api/studentBatches/students/batch/`, {
          user_id: user_id,
  ...
  const handleCreateNewCourse = async () => {
    try {
      const response = await axios.post('http://localhost:3002/api/batches/', {
  ...
  const handleDeleteCourse = async () => {
    try {
      await axios.delete(`http://localhost:3002/api/batches/${batchToDelete}`);
