...
const StudentMarks = () => {
  ...
  useEffect(() => {
    const fetchTestDetails = async () => {
      try {
        ...
        const testResponse = await fetch(`http://localhost:3002/api/test/${testId}`);
        if (testResponse.ok) {
    ...
    const fetchStudentsInBatch = async (batchId) => {
      try {
        const batchResponse = await fetch(`http://localhost:3002/api/studentbatches/students/batch/${batchId}`);
        if (batchResponse.ok) {
    ...
    const fetchStudentTestRecords = async (studentsData) => {
      try {
        const marksResponse = await fetch(`http://localhost:3002/api/studenttestrecords/test/${testId}`);
        let marksData = [];
        ...
        const recordsWithNames = await Promise.all(studentsData.map(async (student) => {
          const userResponse = await fetch(`http://localhost:3002/api/users/user/${student.user_id}`);
    ...
    const createStudentTestRecord = async (userId) => {
      try {
        const newRecordResponse = await fetch('http://localhost:3002/api/studenttestrecords/', {
          method: 'POST',
  ...
  const updateStudentMarks = async (recordId, marks) => {
    try {
      const response = await fetch(`http://localhost:3002/api/studenttestrecords/${recordId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
