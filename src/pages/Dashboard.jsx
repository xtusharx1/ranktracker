...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch roles
        const rolesResponse = await fetch("http://ec2-13-202-53-68.ap-south-1.compute.amazonaws.com:3002/api/role");
        const rolesData = await rolesResponse.json();
  
        // Fetch user counts by role
        const userCountsResponse = await fetch("http://ec2-13-202-53-68.ap-south-1.compute.amazonaws.com:3002/api/users/roles/count");
        const userCountsData = await userCountsResponse.json();
  
        // Fetch course count (optional, if the endpoint exists)
        let courseCount = 0;
        try {
          const courseResponse = await fetch("http://ec2-13-202-53-68.ap-south-1.compute.amazonaws.com:3002/api/courses/count");
